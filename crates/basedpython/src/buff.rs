// mirrors `crates/ruff/src/main.rs` — keep in sync. the `buff` linter /
// formatter entrypoint, built into the combined `basedpython` wheel
use std::io::Write;
...
fn report_error(err: &anyhow::Error) -> ExitCode {
    {
        ...
        writeln!(stderr, "{}", "buff failed".red().bold()).ok();
