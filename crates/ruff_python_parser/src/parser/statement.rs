    ...
    pub(super) fn parse_statement(&mut self) -> Stmt {
        ...
        match self.current_token_kind() {
            ...
            TokenKind::Class => {
                ...
                Stmt::ClassDef(self.parse_class_definition(vec![], start))
            }
            ...
            token => {
                ...
                // Handle basedpython modifier keywords before class/def:
                //   final class/def, abstract class/def, override def, open class,
                //   static def, data class, frozen data class, enum class,
                //   export class/def, public class/def, private class/def
                // Also: let x = 5, newtype Foo = int
                if token == TokenKind::Name {
                    ...
                    let is_modifier = match kw {
                        ...
                        "data" | "enum" => next == TokenKind::Class,
                        "frozen" => {
                            let (p1, p2) = self.peek2();
                            p1 == TokenKind::Name && p2 == TokenKind::Class
                        }
                        _ => false,
                    };
                    if is_modifier {
                    ...
                    // `newtype Foo = int` — newtype declaration
                    if kw == "newtype" && next == TokenKind::Name
                        && self.peek2().1 == TokenKind::Equal
                    {
                        return self.parse_newtype_decl(start);
                    }
